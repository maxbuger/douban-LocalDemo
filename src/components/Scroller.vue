  ...
  <div class="scroller">
    ...
    <div class="content">
      ...
      <ul class="hasCover" v-if="type==='hasCover'">
        <li v-for="item in items">
          <router-link :to="'subject/'+item.id" append>
            <img v-if="item.images" :src="item.images.medium">
      ...
      <ul class="onlyString" v-if="type==='onlyString'">
        <li v-for="item in items" style="border-color:#9F7860;">
          <a href="" :href="item.href" v-if="!item.line" :style="{color:item.color}">{{item.title}}</a>
        </li>
      </ul>
    </div>
  </div>
